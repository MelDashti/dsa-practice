...
1. Any sequence of consecutive cards of length group_size.
   - For example, if she has cards [17,13,11,2,3,5,7] and group_size = 5, one valid group
...
of length group_size, or return false otherwise.
...
- 1 <= hand.length <= 10^4
- 0 <= hand[i] <= 10^9
- 1 <= group_size <= hand.length

Approach (Greedy with Counting):
1. Check if total cards divisible by group_size
...
class Solution:
    def is_n_straight_hand(self, hand: List[int], group_size: int) -> bool:
        ...
        """
        # Base check: total cards must be divisible by group_size
